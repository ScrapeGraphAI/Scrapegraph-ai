...
class TestScriptCreatorMultiGraph:
    ...
    @pytest.mark.asyncio
    @patch('scrapegraphai.graphs.script_creator_multi_graph.BaseGraph')
    async def test_custom_model_token_limit(self, mock_base_graph):
        """
        Test that a custom model token limit is correctly set and passed to the graph
        when initializing ScriptCreatorMultiGraph.
        """
        # Arrange
        prompt = "What is Chioggia famous for?"
        ...
        # Check if BaseGraph was initialized with the correct configuration
        mock_base_graph.assert_called_once()
        _, kwargs = mock_base_graph.call_args
        assert 'config' in kwargs
        assert kwargs['config']['model_token'] == custom_token_limit